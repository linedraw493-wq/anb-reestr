...
def test_dlya_svoego_nomera_rabotaet(monkeypatch):
    _nastroit(monkeypatch, "111111", "+77010000000")
    assert vhod.master_kod_dlya("+77010000000") is True


def test_nomer_uznayotsya_v_lyuboy_zapisi(monkeypatch):
    """Владелец пишет свой номер и через 8, и через +7 — это один человек."""
    _nastroit(monkeypatch, "111111", "87010000000")
    assert vhod.master_kod_dlya("+77010000000") is True
    assert vhod.master_kod_dlya("87010000000") is True
    assert vhod.master_kod_dlya("7 701 000 00 00") is True
...
def test_pustoy_spisok_znachit_nikomu(monkeypatch):
    ...
    assert vhod.master_kod_dlya("+77010000000") is False
...
def test_bez_koda_spisok_nichego_ne_znachit(monkeypatch):
    _nastroit(monkeypatch, "", "+77010000000")
    assert vhod.master_kod_dlya("+77010000000") is False
